...
class ControllerBoutique {
    ...
    public static function achatItem() {
        ...
        $req = Model::getPDO()->prepare(" SELECT * FROM PE__Objet JOIN PE__Inventaire ON PE__Objet.itemID = PE__Inventaire.itemID WHERE PE__Objet.itemID = :itemID AND PE__Inventaire.userID= :userID");
        $req->execute(array('userID' => $userID, 'itemID' => $itemID));
        $req = $req->fetch();
        if(!$req){
